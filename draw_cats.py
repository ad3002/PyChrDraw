...
if __name__ == '__main__':
    ...
    wild_sizes = settings["wild_sizes"]
    ...
    print "Wild cat"
    wild_chr2pos2freq, distribution = compute_interval_freq(wild_chr2bands, wild_sizes, bp_per_pixel=100000)
    wild_chr2pos2freq = normalize_interval_freq(wild_chr2pos2freq, distribution, max_cutoff=210)
    wild_bands = freqs_to_bands(wild_chr2pos2freq)

    print "Fix coordinates..."
    ...
    w_ready_chrs, w_chr2bands = normalize_chromosome_sizes(wild_sizes, real_chr_size, bands=wild_bands)
    ...
    for key in keys:
            ...
            length = w_ready_chrs[k]
            draw_horizontal_chromosome(draw, x, y, length, bands=w_chr2bands[k], chr_width=chr_width, name=key, scale=1, stars=None)
    

            y += space_between_chr + 20

    im.save("100k_cat_70_wild_210.png")